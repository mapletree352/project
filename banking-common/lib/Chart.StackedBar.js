(function (factory) {
	...
	Chart.Type.extend({
		...
		draw : function(ease){
			...
			//Don: Initialize an array to store the total
			var totals = [];
			for (var i = 0; i < this.datasets[0].bars.length; i++) {
				totals[i] = 0;	
			}
			...
			//Don: To display total on top of each bar
			...
			for (var i = 0; i < dataset.bars.length; i++) {
				...
				ctx.fillText(totals[i], bar.x, bar.y);
			}
		}
	});
}));
